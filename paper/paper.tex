...
A Visualization system must be fast enough to enable fast iterations; with delays of more than 500ms analysts become less effective ~\cite{liu2014effects} or even lose their flow of though if delays are more than a second~\cite{nielsen1993response}. 
...
The HCI community found that approximate visualizations can be used in exploratory data analsis~\cite{fisher2012trust, moritz2017pangloss}.
...
Moreover, in many models in the real world no statistial model is available to estimate the uncertaintly so that the approximate visualizations in our study do not show uncertainties.
...
Our study flow (\figref{fig:example}) aimed to mimic this experience. %TODO: add; probably include a few screenshots about what participants actually saw.
