...
In the study, we randomized the airline and state to focus on.
...
\todo{Talk about our participants.}
...
We randomized visualization order, approximate sample, focus state, focus airline, and study condition for each participant.
As previously stated, the variation level of the approximate visualizations varied by visualization.

\subsubsection{Dependent Variables}

We asked three types of questions about each precise visualization, bulding on Brehmer and Munzner's task taxonomy \cite{brehmer2013multi}.
...
For question (3), the answers were calculated via a Jaccard Index.
...
\caption{Regression table for Question 1. Baseline is only the precise visualization.}
\label{table_example}
\centering
...
Condition:Both & & & & \\
\hline
Expected Bias * Condition:Difference & & & & \\
\hline
Expected Bias * Condition:Both & & & & \\
\hline
\end{tabular}
\end{table}
