...
\begin{figure*}[!ht]
\centering
\subfloat[Approximate visualization based on 0.2\% of the data. Note that ``New Yersey'' is missing and that ``Florida'' is the state with the most outgoing flights.]{\includegraphics[width=0.3\textwidth]{states5009.png}%
\label{fig:approx}}
\hfil
...
\label{fig:diff}}
\caption{Visualizations of aproximate and precise data of the number of flight per state. (c) highlights the difference to the approximate data.}
\label{fig:types}
\end{figure*}
...
To visually separate visualizations of approximate and precise data, we colored the bars approximate visulizations orange (\figref{fig:approx}) and prcise visualizations blue (\figref{fig:precise}). 
...
\begin{enumerate}
\item Was there a difference between the precise and approximate visualizations (categorial dependent variable)
\item Which airline or state had the most or least flights (categorical dependent variable)
\item How many flights were on a particular airline or from a particular state (interval depdendent variable)
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=2.5in]{blankheatmap.png}
