...
Precise results considering the entire dataset are either updated progressively \cite{fisher2012trust} or asynchronously \cite{moritz2017pangloss} \cite{kay2016ish}.
...
This work considers the perceptual question of whether temporary exposure imprecise visualizations in an exploratory data analysis process influences people's recognition of the data.
We further explore whether the design of the precise visualization can the impact of the temporary exposure to imprecise data.
...
\item Is this impact mitigated by highlighting the change from the imprecise visualization in the precise visualiztion?
\end{enumerate}
...
Through an X-person experiment on Amazon Mechanical Turk, we found...

\section{Related Work}\label{sec:related_work}

This paper builds from literature on exploring large datasets, visualizing uncertainty, and studies of human perception of visualizations.

I'm punting on this section. Dominik, all you :-)

\subsection{Exploring Large Datasets through Interactive Visualization}

\cite{moritz2017pangloss} \cite{fisher2012trust}

\subsection{Visualizing Uncertainty}

\cite{kay2016ish}

\subsection{Understanding Human Perception of Visualization}

\cite{brehmer2013multi} \cite{heer2010crowdsourcing}
Things like Weber's law, Error Bars Considered Harmful

\section{Experimental Setup}\label{sec:experimental_setup}

We sought to create an experiment which mimiced the experience of Exploratory Data Analysis with an approximate query system while still maintaining control over participant experiences.
...
Our study flow (Figure X) aimed to mimic this experience. %TODO: add; probably include a few screenshots about what participants actually saw.
...
After viewing both precise visualizations, participants answered a larger set of questions based on Brehmer and Munzner's work \cite{brehmer2013multi}.
...
Participants were informed that first two visualizations were approximations of their data, allowing us to mimic the experience of looking at an approximate query result.
%Do we want to go into more detail on the questions? In a 'dependent variables' section?

The two visualizations participants saw were based on the same dataset about flights in the United States. \url{http://datahub.io/dataset/open-flights}
Both visualizations were bar graphs, which are common in approximate query systems \cite{moritz2017pangloss}.
The y-axis varied between the two visualizations, showing either the state the flight left from or the airline of the flight.
...
Our control condition presented just the updated, precise data, as in PanGloss and other systems \cite{moritz2017pangloss}.
We further tested presenting the approximate and precise visualizations side-by-side, and a visualization highlighting the approximate data on the precise visualization.

We varied how much of the dataset was sampled for each approximate visualization.
We tested sample levels of 0.1\%, 0.2\%, 0.5\%, and 1\%, with an additional control condition of 100\% (e.g., the approximate visualization was identical to the precise visualization).
...
For example, if a participant was asked "How many flights were there on Alaska Airlines?", we record the difference in flights between the approximate and precise visualization.
...
Tie to crowdsourcing literature here, about what our work could inform over there.

\subsubsection{Threads to Validity} %I would opt to put this under the experimental setup, but I understand that Mike might want differently.
